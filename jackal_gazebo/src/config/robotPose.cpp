...
const vector<geometry_msgs::PoseStamped> & RobotPose::getWheelsPoses()
{
    ...
    try{
        ...
        listener->lookupTransform("/minefield", "/rear_left_wheel_link", ros::Time(0), transform);
    }
    catch (tf::TransformException &ex) {
//        ROS_ERROR("%s",ex.what());
    ...
    try{
        // faster lookup transform so far
        listener->lookupTransform("/minefield", "/rear_right_wheel_link", ros::Time(0), transform);
    }
    catch (tf::TransformException &ex) {
//        ROS_ERROR("%s",ex.what());
        // 30 Hz -- maybe too fast
    ...
    try{
        // faster lookup transform so far
        listener->lookupTransform("/minefield", "/front_left_wheel_link", ros::Time(0), transform);
    }
    catch (tf::TransformException &ex) {
//        ROS_ERROR("%s",ex.what());
        // 30 Hz -- maybe too fast
    ...
    try{
        // faster lookup transform so far
        listener->lookupTransform("/minefield", "/front_right_wheel_link", ros::Time(0), transform);
    }
    catch (tf::TransformException &ex) {
//        ROS_ERROR("%s",ex.what());
        // 30 Hz -- maybe too fast
